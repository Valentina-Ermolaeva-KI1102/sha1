...
uchar* sha1(std::ifstream *in)
{
	...
	char buf;
	uint A = 0x67452301, B = 0xEFCDAB89, C = 0x98BADCFE, D = 0x10325476, E = 0xC3D2E1F0, AA, BB, CC, DD, EE;
		...
		for(i = 0; i < size; i++)
		{
			...
			if(in->eof())
			{
				if(i < 55)
				{
					...
					for(i++; i < 56; i++)
						msg_for_hash[i] = 0;
					_int64 bit_length = (uint)(lenght)*8; // lounge of massage into bit
...
int main(int argc, char **argv)
{
	...
	uchar *SHA1 = sha1(&in);
	for(int i = 0; i < 20; i++)
		out<<std::hex<<std::uppercase<<uint(SHA1[i])<<" ";
	in.close();
	out.close();
	return 0;
}
